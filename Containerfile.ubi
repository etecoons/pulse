# =============================================================================
# pulse — Alpine container image (Unraid-compatible)
# =============================================================================
...
#
# Build (Podman):
#   cd ~/projects/studio2201/pulse
#   podman build -f Containerfile.ubi -t pulse:latest .
#
# Run (Unraid-style):
#   podman run -d --name pulse \
#     -p <host-port>:4406 \
#     -v /mnt/user/appdata/pulse:/config \
#     -e PULSE_PIN=<your-pin> \
#     pulse:latest
#

# Compare size:
#   podman images | grep -E 'pulse|studio2201/pulse'
# =============================================================================
...
LABEL org.opencontainers.image.title="pulse" \
      org.opencontainers.image.description="pulse (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/pulse" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/pulse
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4406}/health" >/dev/null || exit 1
